...
import { ai } from '@zerostep/playwright';
  ...
  test('Can_Self_Register_Sandy_Prior', async ({ page }) => {
    ...
    await ai('Scroll down to "Collection notice"', { page, test })
    //await ai('Click on "Register to get a family doctor or nurse practitioner" text', { page, test })
    await ai('Scroll down to very end of the same page', { page, test })
    //await ai('Click on "Collection notice" text on the same page', { page, test })
    //await ai('Click the "Next" button on the same page', { page, test })
    //await ai('Scroll down and click', { page, test })
